...
func (asset *Asset) CreateRPCClient() error {
	...
	if utils.GlobalItem.Testnet {
		port = beego.AppConfig.String("DCR_WALLET_TESTNET_PORT")
		asset.ChainParam = chaincfg.TestNet3Params()
	} else {
		port = beego.AppConfig.String("DCR_WALLET_MAINNET_PORT")
...
func (asset *Asset) GetSystemLabel() string {
	if utils.GlobalItem.Testnet {
		return beego.AppConfig.String("DCR_SYSTEM_TESTNET_ACCOUNT")
	}
	return beego.AppConfig.String("DCR_SYSTEM_ACCOUNT")
}
...
func (asset *Asset) SetSystemAddress() {
	systemAddress, _ := utils.GetSuperadminSystemAddress(assets.DCRWalletAsset.String())
