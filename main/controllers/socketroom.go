...
func init() {
	...
	go func() {
		for {
			//Get rate info
			var rateJsonStr string
			res, err := services.FetchRateHandler(context.Background())
			if err != nil {
				rateJsonStr = "{}"
			} else {
				rateJsonStr = res.Data
			}
...
func SocketEventArea() {
	for {
		...
		case event := <-publish:
			...
			if event.Type == models.EVENT_CHATMSG {
				fmt.Println("Message from userId: ", event.UserId)
			}
