...
export default class extends BaseController {
  ...
  async initialize() {
    this.chatActive = this.data.get("chatActive");
    ...
    this.socket = new WebSocket(
      method + "://" + window.location.host + "/ws/connect"
    );
  ...
  handlerChatSocketSignal(content) {
    ...
    var isMyValidUpdateChat =
      (content.fromName == _this.loginName ||
        content.toName == _this.loginName) &&
      chatContent.userName != _this.loginName;
    //if is not my chat, return
    if (!isMyValidUpdateChat) {
      return;
    }
    var targetName =
      content.fromName == _this.loginName ? content.toName : content.fromName;
    var hasUpdate = false;
    var updateIndex = -1;
    ...
    if (updateIndex == _this.currentChatIndex && _this.chatboxDisplay) {
      ...
      (!_this.chatboxDisplay || (_this.chatboxDisplay && !isCurrent)) &&
      !hasUnread
    ) {
      _this.unreadCount++;
    ...
    if (isCurrent) {
      $("#chatContentArea").append(
        _this.createChatContentHTML(
          _this.chatList[_this.currentChatIndex],
          chatContent
        )
      );
  ...
  createChatContentHTML(chatMsg, element) {
    ...
    if (element.isHello) {
      innerHTML +=
        `<div class="no-gutters text-center c-b-grey">` +
        `<span class="fs-13">${this.GetChatTime(
          Number(element.createdt)
        )}</span><br>` +
        `<span class="fs-14">${element.content}</span></div>`;
    } else {
      innerHTML +=
        `<div class="no-gutters d-flex justify-content-${
          isTarget ? "start" : "end"
        }">` +
        `<div class="chat-bubble chat-bubble--${
          isTarget ? "left" : "right"
        } mw-70 minw-40">` +
        `<div class="d-flex justify-content-between"><span class="fw-600 fs-14">${
          element.userName
        }</span><span class="fs-13">${this.GetChatTime(
          Number(element.createdt)
        )}</span></div>${element.content}</div>` +
        `</div>`;
    }
    return innerHTML;
  }
    ...
    $(".floatingMenu")
      ...
      .slideToggle(200, function () {
        ...
        if ($("#floatingMenu").css("display") == "none") {
          ...
          $("#chatContentArea").scrollTop(
            $("#chatContentArea")[0].scrollHeight
          );
        }
      });
  }
  ...
  initChatMsgList() {
    ...
    this.chatList.forEach((element, index) => {
      if (element.hasContent || element.newMsg) {
        ...
        innerHtml +=
          `<div id="chatItem${index}" class="friend-drawer friend-drawer--onhover ${
            !_this.isMobileView && _this.currentChatIndex == index
              ? "chat-active"
              : ""
          }" data-chat-index-param="${index}" data-action="click->chat#chatMsgChange">` +
          `<div ${
            element.unreadNum > 0
              ? 'data-after-text="' +
                element.unreadNum +
                '" data-after-type="red badge top left"'
              : ""
          }><img class="profile-image" src="/static/images/avatar.png" alt=""></div>` +
          `<div class="text">` +
          `<div class="d-flex justify-content-between"><span class="fw-600">${
            element.targetUser
          }</span><span id="itemTime${index}" class="time text-muted small">${
            !lastItem ? "" : _this.GetChatTime(Number(lastItem.createdt))
          }</span></div>` +
          `<p class="text-muted elipse-text" id="itemLastContent${index}">${
            !lastItem ? "" : lastItem.content
          }</p>` +
          `</div>` +
          `</div>`;
      }
  ...
  updateChatMsgContent(lastContent, index) {
    $("#itemTime" + index).text(
      !lastContent ? "" : this.GetChatTime(Number(lastContent.createdt))
    );
    ...
    $.ajax({
      ...
      success: function (res) {
        if (!res.error) {
          ...
          $("#chatContentArea").append(
            _this.createChatContentHTML(chatMsg, msgContent)
          );
          //reset input field
          $("#msgContent").val("");
