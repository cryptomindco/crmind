...
func (ts *TransactionService) GetExchangePrice(currency string) (float64, error) {
	//get exchange price on config
	exchangeConfig := beego.AppConfig.String("EXCHANGE")
	...
	case Binance:
		return ts.GetBinancePrice(currency)
	case Kucoin:
		return ts.GetKucoinPrice(currency, "USDT")
	case Mexc:
		return ts.GetMexcPrice(currency, "USDT")
	default:
		return 0, fmt.Errorf("%s", "Exhchange failed!")
	}
}

func (ts *TransactionService) GetExchangeMultilPrice(currencies []string) (map[string]float64, error) {
	//get exchange price on config
	exchangeConfig := beego.AppConfig.String("EXCHANGE")
	if utils.IsEmpty(exchangeConfig) {
	...
	switch exchangeConfig {
	case Binance:
		return ts.GetBinanceMultilPrice(currencies)
	case Kucoin:
		return ts.GetKucoinMultilPrice(currencies)
	case Mexc:
		return ts.GetMexcMultilPrice(currencies)
	default:
		return nil, fmt.Errorf("%s", "Exhchange failed!")
	}
}

func (ts *TransactionService) GetKucoinPrice(firstCurrency string, secondCurrency string) (float64, error) {
...
func (ts *TransactionService) GetMexcPrice(firstCurrency string, secondCurrency string) (float64, error) {
	var symbol = fmt.Sprintf("%s%s", strings.ToUpper(firstCurrency), strings.ToUpper(secondCurrency))
	query := map[string]string{
		"symbol": symbol,
	}
...
func (*TransactionService) GetBinancePrice(currency string) (float64, error) {
...
func (*TransactionService) GetBinanceMultilPrice(currencies []string) (map[string]float64, error) {
...
func (ts *TransactionService) GetKucoinMultilPrice(currencies []string) (map[string]float64, error) {
	result := make(map[string]float64)
	for _, currency := range currencies {
		if currency == assets.USDWalletAsset.String() {
			continue
		}
		price, err := ts.GetKucoinPrice(currency, "USDT")
		if err != nil {
			return nil, err
		}
...
func (ts *TransactionService) GetMexcMultilPrice(currencies []string) (map[string]float64, error) {
	result := make(map[string]float64)
	for _, currency := range currencies {
		if currency == assets.USDWalletAsset.String() {
			continue
...
func (*TransactionService) GetSymbolByExchange(exchange string, currency1 string, currency2 string) string {
	switch exchange {
	case Kucoin:
...
func (ts *TransactionService) GetAllSymbolFromChainList(currencies []string, exchange string) ([]string, []string) {
	...
	for _, currency := range currencies {
		for _, currency2 := range currencies {
			...
			var symbol = ts.GetSymbolByExchange(exchange, strings.ToUpper(symbol1), strings.ToUpper(symbol2))
...
func (this *TransactionService) HanlderExchangeRateCompensation(usdRateMap map[string]float64, allRateMap map[string]float64, currencies []string, exchange string) map[string]float64 {
	result := make(map[string]float64)
	for _, currency := range currencies {
		for _, currency2 := range currencies {
			if currency2 == currency {
			...
			var symbol = this.GetSymbolByExchange(exchange, symbol1, symbol2)
...
func (this *TransactionService) GetBinanceAllMultilPrice(currencies []string) (map[string]float64, map[string]float64, error) {
	//Create all symbol from input currencies
	usdSymbol, allSymbol := this.GetAllSymbolFromChainList(currencies, Binance)
	req := &dohttp.ReqConfig{
		Method:  http.MethodGet,
		HttpUrl: binancePriceURL,
	...
	result = this.HanlderExchangeRateCompensation(usdRateMap, result, currencies, Binance)
	return lowerUsdRateMap, result, nil
}

func (this *TransactionService) GetKucoinAllMultilPrice(currencies []string) (map[string]float64, map[string]float64, error) {
	//Create all symbol from input currencies
	usdSymbol, allSymbol := this.GetAllSymbolFromChainList(currencies, Kucoin)
	req := &dohttp.ReqConfig{
		Method:  http.MethodGet,
	...
	result = this.HanlderExchangeRateCompensation(usdRateMap, result, currencies, Kucoin)
	return lowerUsdRateMap, result, nil
}

func (ts *TransactionService) GetMexcAllMultilPrice(currencies []string) (map[string]float64, map[string]float64, error) {
	//Create all symbol from input currencies
	usdSymbol, allSymbol := ts.GetAllSymbolFromChainList(currencies, Mexc)
	req := &dohttp.ReqConfig{
		Method:  http.MethodGet,
		HttpUrl: mexcPriceURL,
		Payload: map[string]string{},
	...
	result = ts.HanlderExchangeRateCompensation(usdRateMap, result, currencies, Mexc)
	return lowerUsdRateMap, result, nil
}

// Get all rate. Return : usdratemap, allratemap, error
func (ts *TransactionService) GetAllMultilPrice(currencies []string) (map[string]float64, map[string]float64, error) {
	//get exchange price on config
	exchangeConfig := beego.AppConfig.String("EXCHANGE")
	if utils.IsEmpty(exchangeConfig) {
	...
	switch exchangeConfig {
	case Binance:
		return ts.GetBinanceAllMultilPrice(currencies)
	case Kucoin:
		return ts.GetKucoinAllMultilPrice(currencies)
	case Mexc:
		return ts.GetMexcAllMultilPrice(currencies)
	default:
