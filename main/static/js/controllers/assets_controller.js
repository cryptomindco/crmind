...
export default class extends BaseController {
  ...
  loadHistoryList() {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        //handler pagination
        _this.paginationTopBarTarget.innerHTML = _this.createPaginationBar(pageCount)
        _this.paginationBottomBarTarget.innerHTML = _this.createPaginationBar(pageCount)
  ...
  createPaginationBar(pageCount) {
    let inner = `<div class="pagination-btn ${this.queryParams.pageNum == 1 ? 'pagination-btn-disabled' : ''}"><a class="pagination-prev-next-btn" data-action="click->assets#prevPageClick"><span class="material-symbols-outlined">chevron_left</span></a></div>`
    for(let i=1;i<=pageCount;i++) {
      inner += `<div data-action="click->assets#pageNumberClick" data-assets-pagenumber-param="${i}" class="pagination-btn${this.queryParams.pageNum == i ? ' active-btn' : ''}"><a>${i}</a></div>`
    }
    inner += `<div class="pagination-btn ${this.queryParams.pageNum == pageCount ? 'pagination-btn-disabled' : ''}"><a class="pagination-prev-next-btn" data-assets-pagecount-param="${pageCount}" data-action="click->assets#nextPageClick"><span class="material-symbols-outlined">chevron_right</span></a></div>`
    return inner
  }
