...
func (asset *Asset) CreateRPCClient() error {
	...
	if utils.GlobalItem.Testnet {
		port = asset.Config.LtcTestnetPort
		walletName = asset.Config.LtcSystemTestnetWallet
		walletPassphrase = asset.Config.LtcTestnetWalletPassphrase
		asset.ChainParam = &chaincfg.TestNet4Params
	} else {
		port = asset.Config.LtcMainetPort
		walletName = asset.Config.LtcSystemWallet
		walletPassphrase = asset.Config.LtcWalletPassphrase
		asset.ChainParam = &chaincfg.MainNetParams
	}

	host := asset.Config.LtcRpcHost
	rpcUser := asset.Config.LtcRpcUser
	rpcPass := asset.Config.LtcRpcPass
...
func (asset *Asset) GetWalletPassphrase() string {
	if !utils.GlobalItem.Testnet {
		return asset.Config.LtcWalletPassphrase
	}
	return asset.Config.LtcTestnetWalletPassphrase
}
...
func (asset *Asset) SetSystemAddress() {
	systemAddress, _ := asset.Handler.GetSuperadminSystemAddress(assets.LTCWalletAsset.String())
